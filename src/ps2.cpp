...
/*
#define INT16_TO_BINARY_PATTERN "%c%c%c%c-%c%c%c%c-%c%c%c%c-%c%c%c%c"
#define PS_TO_BINARY_PATTERN    "(%c%c)-%c%c%c%c-%c%c%c%c-(%c)%c%c%c%c%c"
#define INT16_TO_BINARY(int16)  \
(((int16>>8)&0xFF)& 0x80 ? '1' : '0'), \
(((int16>>8)&0xFF) & 0x40 ? '1' : '0'), \
(((int16>>8)&0xFF) & 0x20 ? '1' : '0'), \
(((int16>>8)&0xFF) & 0x10 ? '1' : '0'), \
(((int16>>8)&0xFF) & 0x08 ? '1' : '0'), \
(((int16>>8)&0xFF) & 0x04 ? '1' : '0'), \
(((int16>>8)&0xFF) & 0x02 ? '1' : '0'), \
(((int16>>8)&0xFF) & 0x01 ? '1' : '0'), \
(int16 & 0x80 ? '1' : '0'), \
(int16 & 0x40 ? '1' : '0'), \
(int16 & 0x20 ? '1' : '0'), \
(int16 & 0x10 ? '1' : '0'), \
...
#define BIT11_BINARY_PATTERN "%c%c%c-%c%c%c%c-%c%c%c%c"
#define BIT11_TO_BINARY(int16)  \
(((int16>>8)&0xFF) & 0x04 ? '1' : '0'), \
(((int16>>8)&0xFF) & 0x02 ? '1' : '0'), \
(((int16>>8)&0xFF) & 0x01 ? '1' : '0'), \
(int16 & 0x80 ? '1' : '0'), \
...
(int16 & 0x02 ? '1' : '0'), \
(int16 & 0x01 ? '1' : '0') 
void KeyboardPS2::d_sleep_us(uint us){
	for(uint i=0;i<us;i++){
		for(int j=0;j<25;j++){
			us_val++;
		}
...
void KeyboardPS2::KeyboardHandler(uint /*gpio*/, uint32_t /*event_mask*/){
	uint8_t repeat=3;
	if ((ps2Data.bitcount > 10)&&((time_us_32()-ps2Data.last_key)>1000)){
		ps2Data.bitcount=0;
		ps2Data.data = 0x00;
		ps2Data.raw_data = 0x00;
		ps2Data.parity = 0;
		ps2Data.last_key=time_us_32();
		//printf("RT[%02d][%02X]>%d>%d>%d\n",ps2Data.head,ps2Data.kbd_buffer[ps2Data.head],ps2Data.bitcount,time_us_32(),ps2Data.last_key);
	};

	ps2Data.raw_data >>= 1; //читаем 11 бит в INT16 сдвигая слева на право побитно
	if(gpio_get(KBD_DATA_PIN)>0){
		ps2Data.raw_data|= 0x8000;
	}
	if ((ps2Data.bitcount > 0) && (ps2Data.bitcount < 9)){
		ps2Data.parity+=gpio_get(KBD_DATA_PIN);
	}

	if ((ps2Data.bitcount++ == 10)) { //11 бит?
		uint8_t i = ps2Data.head + 1;
		if (i >= KBD_BUFFER_SIZE) i = 0;
		if (i != ps2Data.tail) {
			uint32_t temp = ps2Data.raw_data;
			while(repeat>0){
				temp>>=5; //сдвигаем вправо до нулевого принятого бита
				if((temp&0x01)^1){ //стартовый бит=0?
					temp>>=1; //пропускаем стартовый бит
					ps2Data.data=(temp&0xFF); //выколупываем скан код
					temp>>=8; //пропускаем данные до бита чётности и стопового бита
					ps2Data.parity&=1;
					//printf("parity:%d>%d\n",ps2Data.parity,((temp&0x01)^1)); //&&()
					if(((temp&0x02)>0)&&(((temp&0x01)^1)==ps2Data.parity)){ //стоповый бит == 1? И четность совпала?
						ps2Data.kbd_buffer[i] = ps2Data.data; //сохраняем данные
						ps2Data.head = i;
						return;
					} else{
						temp = ps2Data.raw_data<<1; //стоповый бит не равен единице - сдвигаем данные на бит вправо и пытаемся перечитать.
						repeat--; // минус 1  попытка распознавания
					}
				} else{
					temp = ps2Data.raw_data<<1; //стартовый бит не равен нулю - сдвигаем данные на бит вправо и пытаемся перечитать.
					repeat--; // минус 1  попытка распознавания
				}
			}
			//printf("BU[%02d][%02X]>%d>%d>%d\n",i,ps2Data.kbd_buffer[i],ps2Data.bitcount,time_us_32(),ps2Data.last_key);//4635 us //us_to_ms(time_us_32())
			//ps2Data.data = 0x00;
			//d_sleep_us(25);
			return;
		}
...
uint8_t __not_in_flash_func(KeyboardPS2::get_scan_code)(void){ //__not_in_flash_func()
	if((time_us_32()-ps2Data.last_key)<1000) return 0;
	if (ps2Data.tail == ps2Data.head) return 0;
	//d_sleep_us(25);
	uint8_t c, i;
	i = ps2Data.tail;
	i++;
	if (i >= KBD_BUFFER_SIZE) i = 0;
	c = ps2Data.kbd_buffer[i];
	ps2Data.tail = i;
	//printf("SC[%02d][%02d][%02X]\n",ps2Data.head,ps2Data.tail,ps2Data.kbd_buffer[i]);
	return c;
}

bool KeyboardPS2::decode_kbd(){
	...
	/*if((time_us_32()-ps2Data.last_key)>500){
		ps2Data.bitcount = 0;
	}*/
	return false;
}

void KeyboardPS2::translate_scancode(uint8_t code,bool is_press, bool is_e0,bool is_e1){
	if (is_e1){
...
void KeyboardPS2::Init_kbd(void){
	ps2Data.bitcount=0;
	ps2Data.data=0;
	ps2Data.head=0;
	ps2Data.tail=0;
	memset(ps2Data.kbd_buffer,0,KBD_BUFFER_SIZE);
	...
	//gpio_set_irq_enabled_with_callback(KBD_CLOCK_PIN, GPIO_IRQ_EDGE_FALL, true, (gpio_irq_callback_t)&KeyboardPS2::keyboard_Handler); //

	gpio_set_irq_enabled(KBD_CLOCK_PIN, GPIO_IRQ_EDGE_FALL, true);
	//gpio_set_irq_enabled(KBD_CLOCK_PIN, GPIO_IRQ_EDGE_RISE, true);
	gpio_set_irq_callback((gpio_irq_callback_t)&KeyboardPS2::KeyboardHandler);
	irq_set_enabled(IO_IRQ_BANK0, true);

};
void KeyboardPS2::Deinit_kbd(void){
	gpio_set_irq_enabled_with_callback(KBD_CLOCK_PIN, GPIO_IRQ_EDGE_FALL, false, nullptr);	//(gpio_irq_callback_t)
    gpio_deinit (KBD_CLOCK_PIN);
    gpio_deinit (KBD_DATA_PIN);	
};

KeyboardPS2::KeyboardPS2(){
	Init_kbd();
	is_present=true;
	printf("KeyboardPS2 Constructor was Called!\n");
};
KeyboardPS2::~KeyboardPS2(){
	Deinit_kbd();
	printf("KeyboardPS2 Destructor was Called!\n");
};


void KeyboardPS2::keys_to_str(char* str_buf,char s_char){
    char s_str[2];
    s_str[0]=s_char;
    s_str[1]='\0';
...
//static
void zx_kb_decode(uint8_t* zx_kbd_statuses){
	//memset(zx_keys_matrix,0,8);
	static uint64_t tmp_zx_kbd_statuses64[32];
	uint8_t* tmp_zx_kbd_statuses8=(uint8_t*)tmp_zx_kbd_statuses64;
	
	uint8_t zx_keys_matrix[8];
	convert_kb_u_to_kb_zx(&kbd_statuses,zx_keys_matrix);
	...
	for(int i=0;i<256;i++){
		...
		//if (zx_kbd_statuses[i]!=(~out8)) zx_kbd_statuses[i]=(~out8);
		tmp_zx_kbd_statuses8[i]=(~out8);
	}
	//для быстрого копирования всего буфера
	
	
	uint64_t* dst_zx_kbd_statuses64=(uint64_t*)zx_kbd_statuses;
	uint64_t* src_zx_kbd_statuses64=tmp_zx_kbd_statuses64;
	for(int i=32;i--;){
		*dst_zx_kbd_statuses64++=*src_zx_kbd_statuses64++;
	}
